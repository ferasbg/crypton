...
class SafetyProperties(HyperProperties):
    """
    Store specifications to compute violations of safety properties, which will be initialized to empty values. Define safety properties meant to assert network object state that adheres to safety constraints i.e. nothing bad is happening.
...
class LivenessProperties(HyperProperties):
    """
    Store specifications to compute violations of liveness properties, which will be initialized to empty values. For context, liveness properties are the eventual instance of a good event happening under certain constraints and conditions.
